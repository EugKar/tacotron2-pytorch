...
class VAELoss(nn.Module):
    ...
    def get_q_y_x(self, z_mu, z_logvar, z_prior_mu, z_prior_logvar):
        '''
        ...
        p_z_y = MultivariateNormal(z_prior_mu,
            covariance_matrix=z_prior_logvar.exp().diag_embed())
    ...
    def forward(self, model_output, latent_params, observed_params,
        ...
        latent_prior_mu, latent_prior_logvar = latent_prior_params
        # Parameters of p(z_o | y_o), dimensions: K_o x D_o
        observed_prior_mu, observed_prior_logvar = observed_prior_params
        ...
        observed_prior_logvar_yo = observed_prior_logvar.gather(0, yo_inds)

        # D_KL(q(z_o | X) || p(z_o || y_o)), dimensions: B
        kl_z_observed = self.kl_multivar_norm_diag(
            observed_mu, observed_logvar,
            observed_prior_mu_yo, observed_prior_logvar_yo)
        ...
        kl_z_latent = self.kl_multivar_norm_diag(
            ...
            latent_prior_logvar.unsqueeze(0).repeat([b, 1, 1]))
